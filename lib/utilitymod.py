...
def rungap50(country, data, prg_params, adjfact, vintage_name, changey, path, tipo, logfile):
    ...
    import numpy as np
    import pandas as pd
#    from utilitymod import find_all
    ...
    if prg_params.loc['dll_path', 'value']:
        lib = ct.CDLL(prg_params.loc['dll_path', 'value'])
    else:
        lib = ct.CDLL(projpath+"dll/"+dllversion)
    ...
    if tipo == 'nawru':
        ...
        #DEBUG if NAWRU forecasted year depends on the horizon
        nf = hor
        #END DEBUG
    else:
        name_vars.append('SR')
...
# Bayes
    ...
    pd.DataFrame(yk).to_excel('yk.xls')
...
# ---------------------------------------------------------
    ...
    if tipo=='nawru':
        ...
        if nml['GAP']['Anchor'][1] < 0:
            # outputdf[country + '_NAWRU'].loc[starty:changey + nf] =
            # unobs[3, 0:nmax - ismax + nf] + adjfact[country.lower()]
...
#        if nawru_series[1965] < -90000.0:
#            nawru_series.loc[1965:1966] = nawru_series[1967]
        with open(logfile, 'a') as f:
            f.write('->PASSED')
        return nawru_series
        ...
        # ------------------------------------------------------------------------
        #             var  = find(cnode, lambda node: node.id == 'TFPT')
        #             vint = find(var, lambda node: node.id == vintage_name)
        #             AnyNode(id='calculation',startyear=ismax,nobs=nmax,
        #                     data=unobs[5,0:nmax],dataexten=unobs[5,nmax:nmax+tp5],
        #                     tfpmodel=None, nawrumodel=nml,parent=vint)
        with open(logfile, 'a') as f:
            f.write('->PASSED')
